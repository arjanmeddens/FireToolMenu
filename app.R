...
#############################################################################################################################################
# LOAD PACKAGES
#- Note that packages our pre-loaded through the manifest file (AM:26-8-18)
...
server <- function(input, output, session) {
 ...
 # -- Any input from dropdown1 will trigger the step options stored in `step` to populate dropdown2
  ...
  output$dropdown2 <- renderUI({
    shinyWidgets::pickerInput("dropdown2", h3("Step (Select your descision Step):"), choices = values.func2(), multiple = T, options = shinyWidgets::pickerOptions(
    actionsBox = TRUE,title = "Please select a Descision Step"))
  })
     
  ## Reactive Dropdown 3 # TOOL
  output$dropdown3 <- renderUI({
    shinyWidgets::pickerInput("dropdown3", h3("Tool (select tool for more Information):"), choices = values.func3(), multiple = T, options = shinyWidgets::pickerOptions(
